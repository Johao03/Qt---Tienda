...
void Tienda::calcular(float subtotal)
{
    // Calcular valores
    m_subtotal += subtotal;
    double iva = m_subtotal * IVA;
    double total = m_subtotal + iva;
...
void Tienda::validarTelefono()
{
    ...
    if (telefono.length() != 10){
        ui->inTelefono->setStyleSheet("background-color: rgb(239, 41, 41);");
    }else{
        ui->inTelefono->setStyleSheet("background-color: rgb(132, 225, 113);");
    }
...
void Tienda::on_inTelefono_editingFinished()
{
    validarTelefono();
}
